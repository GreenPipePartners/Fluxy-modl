    ...
    Decision decision() {
        ...
        if (current == null) {
            return new Decision(
                false,
                503,
                "MODULE_LICENSE_UNAVAILABLE",
                "Fluxy module license is unavailable"
            );
        }
        ...
        if (mode == LicenseMode.Trial) {
            boolean expirationReached = current.getTrialExpirationDate() == null
                || !current.getTrialExpirationDate().after(new java.util.Date());
            return current.isTrialExpired() || expirationReached
